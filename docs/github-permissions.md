...
| Contents: read | Base `patchgate.yml` and base CODEOWNERS | Policy remains ambiguous; never fall back to the PR head |
...
| Administration/metadata read | Rulesets and branch-protection visibility | Native control state is unknown; the snapshot is rejected rather than green |
...
4. Native ruleset or branch-protection visibility must be complete. An active
   decision-bearing native control is normalized and rejected because the
   current evaluator cannot represent it safely.
