...
PatchGate provides an official GitHub Action to deterministically evaluate pull requests against your repository's trusted base policy.
...
| `github-token` | GitHub token for reading metadata | `${{ github.token }}` | String (read-only token) |
...
3. **Minimal Permissions**: The action only requires `contents: read`, `pull-requests: read`, and `checks: write` (if check runs are enabled).
