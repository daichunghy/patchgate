...
| G3 GitHub adapter | Recorded/mock authenticated snapshot flow, bounded requests, source and SHA binding, TOCTOU re-read, redaction, branch-protection check/approval contract, 24 integration tests and a live smoke record for `daichunghy/patchgate#9` | Public PR head `75e1d79` built a complete schema-valid live snapshot and receipt with final status `human_review_required`; missing approval/ownership/linkage evidence remains explicit; active rulesets and merge-group membership remain unsupported |
