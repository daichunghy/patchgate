...
| G3 GitHub adapter | Recorded/mock authenticated snapshot flow, bounded requests, source and SHA binding, TOCTOU re-read, redaction, 19 integration tests and a live smoke record for `daichunghy/patchgate#9` | Live smoke reached GitHub but safely rejected `GITHUB_API_UNSUPPORTED` because branch protection is decision-bearing and unsupported by the scalar contract; merge-group membership and decision-bearing native controls remain explicitly unsupported |
| G4 Action | Root `action.yml`, `src/action/index.ts`, committed ncc bundle, pinned workflows, clean-room bundle verification, idempotent check delivery, consumer fixture smoke and explicit non-ready merge-group handling are public on the hardening branch/PR | Local consumer boundary is verified; no live external consumer E2E, public release or two consenting non-blocking shadow installations |
