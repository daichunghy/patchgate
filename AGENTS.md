...
| G3 GitHub adapter | Recorded/mock authenticated snapshot flow, bounded requests, source and SHA binding, TOCTOU re-read, redaction, branch-protection check/approval contract, 23 integration tests and a live smoke record for `daichunghy/patchgate#9` | Local candidate built a complete schema-valid live snapshot and receipt; the real PR remains `evidence_missing` for missing approval/ownership/linkage evidence; active rulesets and merge-group membership remain explicitly unsupported |
