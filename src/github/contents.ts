...
export async function fetchTrustedBasePolicy(client: GitHubClient, owner: string, name: string, baseSha: string, allowConfirmedAbsence = false, phase: "collection" | "finalization" = "collection"): Promise<BasePolicyResult> {
  ...
  try {
    ...
    if (type !== "file" || encoding !== "base64" || content === undefined || path !== "patchgate.yml" || size === undefined) throw new GitHubAdapterError(makeDiagnostic("GITHUB_RESPONSE_MALFORMED", "The base policy file response had an unexpected encoding, path, or size.", { observation: "policySources", snapshotEvaluable: false, exitCode: 2 }));
    ...
    try { artifact = createTrustedPolicyArtifact(text, { identity: "patchgate.yml", revision: baseSha }, "patchgate.yml"); } catch (error) { throw new GitHubAdapterError(makeDiagnostic("GITHUB_POLICY_INVALID", error instanceof Error ? error.message : "The trusted base policy failed validation.", { observation: "policySources", remediation: "Repair patchgate.yml at the base revision and rerun the snapshot." })); }
